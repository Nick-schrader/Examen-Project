...
<section>
	...
	<form method="post" action="{{ route('profile.adresupdate') }}" class="mt-6 space-y-6">
		...
		<div>
			...
			<x-text-input id="straat" name="straat" type="text" class="block w-full mt-1" :value="old('straat', $straat ?? '')" required />
			<x-input-error class="mt-2" :messages="$errors->get('straat')" />
		</div>

		<div>
			<x-input-label for="huisnummer" :value="'Huisnummer'" />
			<x-text-input id="huisnummer" name="huisnummer" type="text" class="block w-full mt-1" :value="old('huisnummer', $huisnummer ?? '')" required />
			<x-input-error class="mt-2" :messages="$errors->get('huisnummer')" />
		</div>

		<div>
			<x-input-label for="postcode" :value="'Postcode'" />
			<x-text-input id="postcode" name="postcode" type="text" class="block w-full mt-1" :value="old('postcode', $postcode ?? '')" required />
			<x-input-error class="mt-2" :messages="$errors->get('postcode')" />
		</div>

		<div>
			<x-input-label for="woonplaats" :value="'Woonplaats'" />
			<x-text-input id="woonplaats" name="woonplaats" type="text" class="block w-full mt-1" :value="old('woonplaats', $woonplaats ?? '')" required />
